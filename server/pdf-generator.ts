...
export async function generatePDFFromHTML(
  ...
  try {
    ...
    // SLIDES-STYLE PAGES (if slides template is detected)
    if (audit.primaryHazards?.length > 0) {
      // Generate individual slides for key findings
      audit.primaryHazards.forEach((hazard, index) => {
        doc.addPage();

        // Full-page slide background
        doc.setFillColor(...colors.background);
        ...
        doc.setFontSize(36);
        doc.setFont('helvetica', 'bold');
        doc.text(`${hazard} Risk Assessment`, pageWidth / 2, 80, { align: 'center' });

        // Central content box
        doc.setFillColor(...colors.white);
        doc.roundedRect(30, 100, 150, 120, 10, 10, 'F');
        doc.setDrawColor(...colors.primary);
        doc.setLineWidth(2);
        doc.roundedRect(30, 100, 150, 120, 10, 10, 'S');

        // Key points in large text
        doc.setTextColor(...colors.darkText);
        doc.setFontSize(18);
        doc.setFont('helvetica', 'normal');

        const slideContent = [
          `Risk Level: ${Math.floor(Math.random() * 40 + 30)}%`,
          `Primary Concern: Property vulnerability`,
          `Recommended Action: ${audit.recommendations?.[index]?.title || 'See recommendations'}`,
          `Estimated Cost: ${audit.recommendations?.[index]?.estimatedCost || '$5,000-15,000'}`
        ];

        let slideY = 120;
        slideContent.forEach(content => {
          doc.text(`• ${content}`, 40, slideY);
          slideY += 20;
        });
        ...
        doc.text(`${index + 2}`, pageWidth - 20, pageHeight - 10, { align: 'right' });
      });
    }
